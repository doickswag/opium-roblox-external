...
// Helper: validate code format (accepts both old "layuh-" and new "zoinbase-" formats)
bool ConfigSystem::is_valid_code_format(const std::string& code) {
    // New format: "zoinbase-" (9 chars) + 8 uppercase letters = 17 total
    if (code.length() == 17 && code.substr(0, 9) == "zoinbase-" && 
        code.substr(9).find_first_not_of("ABCDEFGHIJKLMNOPQRSTUVWXYZ") == std::string::npos) {
        return true;
    }
    // Old format: "layuh-" (6 chars) + 8 uppercase letters = 14 total (for backward compatibility)
    if (code.length() == 14 && code.substr(0, 6) == "layuh-" && 
...
// Helper: generate a unique code with prefix "zoinbase-" followed by 8 uppercase letters
	...
	try {
		...
		for (const auto& entry : fs::directory_iterator(config_directory)) {
			...
			if (j.contains("code") && j["code"].is_string()) {
				std::string stored_code = j["code"].get<std::string>();
				// Match exact code, or match if user entered "zoinbase-" but config has "layuh-" (or vice versa)
				if (stored_code == code) {
					out_config_name = entry.path().stem().string();
					return true;
				}
				// Handle cross-prefix matching: if user enters "zoinbase-XXXX" but config has "layuh-XXXX"
				if (code.length() >= 9 && code.substr(0, 9) == "zoinbase-" && 
				    stored_code.length() >= 6 && stored_code.substr(0, 6) == "layuh-" &&
				    code.substr(9) == stored_code.substr(6)) {
					out_config_name = entry.path().stem().string();
					return true;
				}
				// Handle reverse: if user enters "layuh-XXXX" but config has "zoinbase-XXXX"
				if (code.length() >= 6 && code.substr(0, 6) == "layuh-" && 
				    stored_code.length() >= 9 && stored_code.substr(0, 9) == "zoinbase-" &&
				    code.substr(6) == stored_code.substr(9)) {
					out_config_name = entry.path().stem().string();
					return true;
				}
			}
...
ConfigSystem::ConfigSystem() {
    ...
    if (appdata_path) {
        config_directory = std::string(appdata_path) + "\\layuh\\config";
        free(appdata_path);
    }
                    ...
                    ImGui::BeginChild("config_list", ImVec2(-1, -30));
                    {
                        for (const auto& config : config_files) {
                            ...
                            if (get_config_code(config, cfg_code)) {
                                ImGui::SameLine();
                                // Replace "layuh-" with "zoinbase-" for display
                                if (cfg_code.substr(0, 6) == "layuh-") {
                                    cfg_code = "zoinbase-" + cfg_code.substr(6);
                                }
