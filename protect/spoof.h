...
#define OverwriteReturnAddress AddressSpoofing::SpoofFunction spoof(_AddressOfReturnAddress());

#ifdef _KERNEL_MODE
#define SPOOF_CALL(ret_type,name) (AddressSpoofing::SafeCall<ret_type,std::remove_reference_t<decltype(*name)>>(name))
#else
#define Obfuscate(name) (AddressSpoofing::SafeCall(name))
#endif
...
#pragma region std::forward
	...
	template <class _Ty>
	constexpr bool is_lvalue_reference_v<_Ty&> = Encrypt(1);
...
namespace AddressSpoofing
{
	class SpoofFunction
	{
	public:
		uintptr_t temp = 0;
		const uintptr_t xor_key = 0xff00ff00ff00ff07;
