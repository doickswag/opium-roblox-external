...
roblox::player gettargetclosesttomousesilent() {
    ...
    const bool useKnockCheck = false;
    const bool useHealthCheck = false;
    const bool useWallCheck = globals::misc::wallcheck;
    const bool allowWallFallback = !useWallCheck; // fallback only when wallcheck is disabled
            const bool useFov = globals::combat::usesfov;
        const float fovSize = globals::combat::sfovsize;
        const float fovSizeSquared = fovSize * fovSize;
    const float healthThreshold = 0.0f;
    ...
    for (auto player : players) {
        ...
        // Wallcheck visibility check
        bool isVisible = true;
        if (useWallCheck) {
            Vector3 targetPos = player.head.get_pos();
            Vector3 camPos = { cameraPos.x, cameraPos.y, cameraPos.z };
            isVisible = wallcheck::can_see(camPos, targetPos);
        }
