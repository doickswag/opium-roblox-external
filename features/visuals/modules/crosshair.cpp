...
namespace crosshair {
    void render() {
        ...
        const char* txt = "Luftwaffle";
        const char* xyz = " | Dev";
        const char* layuhOnly = "Luftwaffle";

        float wordW = ImGui::CalcTextSize(txt).x;
        float xyzW = ImGui::CalcTextSize(xyz).x;
        float layuhOnlyW = ImGui::CalcTextSize(layuhOnly).x;
        float totalW = wordW + xyzW;

        // Always show full text
        bool showFull = true;
        float tweenProgress = 1.0f;
        float currentWordW = wordW;
        float currentXyzW = xyzW;
        float currentTotalW = currentWordW + currentXyzW;

        ImVec2 base{ mpos.x - currentTotalW * 0.5f, mpos.y + showGap + globals::visuals::crosshair_size + 4 };

        // Text directly follows crosshair position without lerp
        ImVec2 layuhPos = base;

        ImU32 colW = IM_COL32(255, 255, 255, 255);
        ImU32 colM = IM_COL32(178, 102, 255, 255); // accent color of the .larp (purple)

        // Always show full text with outline effect
        ImU32 outlineColor = IM_COL32(0, 0, 0, 255);
        
        // Draw outline for "layuh"
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <= 1; ++dy) {
                if (dx || dy) {
                    dl->AddText(ImVec2(layuhPos.x + dx, layuhPos.y + dy), outlineColor, layuhOnly);
                }
            }
        }
        
        // Draw main text for "layuh"
        dl->AddText(layuhPos, colW, layuhOnly);
        
        // Draw outline for ".public"
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <= 1; ++dy) {
                if (dx || dy) {
                    dl->AddText(ImVec2(layuhPos.x + layuhOnlyW + dx, layuhPos.y + dy), outlineColor, xyz);
                }
            }
        }
        
        // Draw main text for ".larp"
        ImU32 accentFadeColor = IM_COL32(178, 102, 255, 255); // Purple color matching ImGui overlay
        dl->AddText(ImVec2(layuhPos.x + layuhOnlyW, layuhPos.y), accentFadeColor, xyz);
