...
void utils::CacheChamsPoly() {
    while (true) {
        if (!globals::visuals::visuals || (!globals::visuals::chams && !globals::visuals::wireframe_chams && !globals::visuals::chams_glow_s && !globals::visuals::chams_highlight_s && !globals::visuals::chams_outline_s)) {
            std::this_thread::sleep_for(std::chrono::microseconds(2000));
            continue;
        }
...
void render_chams_enhanced() {
    if ((!globals::visuals::chams && !globals::visuals::wireframe_chams) || !globals::instances::draw)
        return;
    ...
    ImColor fillColor(
        globals::visuals::chams_fillcolor[0],
        globals::visuals::chams_fillcolor[1],
        globals::visuals::chams_fillcolor[2],
        globals::visuals::chams_fillcolor[3]
    );
    ImColor outlineColor(
        globals::visuals::chams_outline[0],
        globals::visuals::chams_outline[1],
        globals::visuals::chams_outline[2],
        globals::visuals::chams_outline[3]
    );
    ImColor wireColor(
        globals::visuals::wireframe_color[0],
        globals::visuals::wireframe_color[1],
        globals::visuals::wireframe_color[2],
        globals::visuals::wireframe_color[3]
    );
    for (const auto& polygon : polygons_to_draw) {
        if (polygon.vertices.size() < 3)
            continue;
        if (globals::visuals::wireframe_chams) {
            globals::instances::draw->AddPolyline(
                polygon.vertices.data(),
                polygon.vertices.size(),
                wireColor,
                true,
                globals::visuals::wireframe_thickness
            );
        }

        // Filled/Outline/Glow should render when wireframe is OFF, independent of master chams toggle
        if (!globals::visuals::wireframe_chams && globals::visuals::chams_glow_s) {
            ImColor glow_color = fillColor;
            glow_color.Value.w *= 0.9f;
        ...
        if (!globals::visuals::wireframe_chams && globals::visuals::chams_highlight_s) {
        ...
        if (!globals::visuals::wireframe_chams && globals::visuals::chams_outline_s) {
            globals::instances::draw->AddPolyline(
                polygon.vertices.data(),
                polygon.vertices.size(),
                outlineColor,
                true,
                globals::visuals::chams_thickness
            );
        }
    }
...
void visuals::run() {
    ...
    for (auto& player : visuals::espData.cachedPlayers) {
        ...
        if (globals::visuals::wireframe_chams || globals::visuals::chams || globals::visuals::chams_glow_s || globals::visuals::chams_highlight_s || globals::visuals::chams_outline_s)
        {
